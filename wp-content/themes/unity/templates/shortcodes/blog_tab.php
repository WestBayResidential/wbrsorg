...
<div id="tab-blog-<?php echo esc_attr( $atts['id'] ); ?>" data-toggle="blog-tab" class="rows tab-blog<?php echo esc_attr( $add_class ); ?>">
	...
	<?php foreach($sub_cats as $key => $cat ){ ?>
	    <a data-item="#cat-<?php echo esc_attr( $cat->slug.'-'.$cat->term_id; ?>" href="<?php echo esc_url( get_category_link( $cat->term_id ) ); ?>">
	...
	<div class="col-sm-9 category-content">
		<?php foreach($sub_cats as $key => $cat ){ ?>
		<div id="cat-<?php echo esc_attr( $cat->slug.'-'.$cat->term_id ); ?>" class="recent-item row">
			<?php
				wp_reset_query();
				$args = array(
					'post_type' => 'post',
