...
<div
  class="<?php echo apply_filters( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, $css_class, $this->settings['base'], $atts ) ?>">
		...
		<div class="widget">		
			...
			<div class="widget-content teaser_grid_container">
				...
				<ul class="nav nav-tabs categories_filter vc_col-sm-12 vc_clearfix">
					...
					<li><a href="#"
						   data-filter=".grid-cat-<?php echo esc_attr($cat->term_id) ?>"><?php echo esc_html( $cat->name ) ?></a></li>
					<?php endforeach; ?>
				</ul>
				<div class="vc_clearfix"></div>
				<?php endif; ?>
				<ul class="wpb_thumbnails wpb_thumbnails-fluid clearfix row list-unstyled"
					data-layout-mode="<?php echo esc_attr($grid_layout_mode) ?>">
					<?php
					/**
					 * Enqueue js/css
					 * {{
					...
					<?php foreach ( $posts as $post ): ?>
						...
						$block_style = isset( $post->bgcolor ) ? ' style="background-color: ' . esc_attr( $post->bgcolor ) . '"' : '';
						?>
						<li
						  class="isotope-item <?php echo apply_filters( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, $li_span_class, 'vc_teaser_grid_li', $atts ) . $post->categories_css ?>"<?php echo trim( $block_style ); ?>>
						...
						</li> <?php echo trim( $this->endBlockComment( 'single teaser' ) ); ?>
						<?php endforeach; ?>
					<?php else: ?>
					<li class="<?php echo esc_attr( $this->spanClass( 1 ) ); ?>"><?php _e( "Nothing found.", "js_composer" ) ?></li>
					<?php endif; ?>
				</ul>
			</div>
		</div>
	...
	<div class="clear"></div>
</div> <?php echo trim( $this->endBlockComment( '.wpb_teaser_grid' ) ) ?>