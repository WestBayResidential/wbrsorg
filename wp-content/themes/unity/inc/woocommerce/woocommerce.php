...
if(class_exists('WooCommerce')){
	...
	class WPO_Woocommerce{
		...
		public function __construct(){
			if(wpo_theme_options('is-quickview',true)){
				// Ajax Quickview
			...
			add_action( 'wpo_button_display',array('WPO_Woocommerce','renderButton'),20 );


			add_action( 'widgets_init', array($this,'override_woocommerce_widgets'), 15 );

			if( wpo_theme_options( 'is-swap-effect',true ) ){
		...
		public function initScripts(){
			wp_enqueue_script('WPO_quickview_js', WPO_THEME_URI.'/js/woocommerce.js',array(),false,true);
		}

		public function wc_product_per_page($cols){
			return wpo_theme_options('woo-number-page',12);
		}
		...
		?>
			<ul class="display pull-left">
                ...
                <li>
                	<a style="position:relative;" data-query='<?php echo trim( $stringquery ); ?>' data-type="list" <?php echo (($layout=='list')?'class="active"':''); ?> href="#">
                		<i class="fa fa-th-list"></i><span><?php echo __('List',TEXTDOMAIN); ?></span>
                	</a>
                </li>
                <li>
                	<a style="position:relative;" data-query='<?php echo trim( $stringquery ); ?>' data-type="grid" <?php echo (($layout=='grid')?'class="active"':''); ?> href="#">
