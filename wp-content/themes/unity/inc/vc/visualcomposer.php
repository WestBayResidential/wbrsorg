	...
	class WPO_VisualComposer extends WPO_PageBuilder_Base{
		...
		private function loadThemeShortCode(){
			...
			if(WPO_CROWDFUNDING_ACTIVED){
				require_once( WPO_THEME_SUB_DIR.'vc/campaign.php' );
			}
			if(WPO_EVENT_ACTIVED){
				require_once( WPO_THEME_SUB_DIR.'vc/event.php' );
			}
		}
		...
		private function elementColumn(){
			$add_css_animation = array(
				"type" => "dropdown",
				"heading" => __("CSS Animation", TEXTDOMAIN),
				"param_name" => "css_animation",
				"admin_label" => true,
				"value" => $this->cssAnimation,
				"description" => __("Select animation type if you want this element to be animated when it enters into the browsers viewport. Note: Works only in modern browsers.", TEXTDOMAIN)
			);
		...
		private function modifyPostGrid(){
			vc_add_param( 'vc_posts_grid', array(
					"type" => "dropdown",
					"heading" => __("Layout Type", TEXTDOMAIN),
					"param_name" => "layout",
					"value" => array(
							'Grid 1'=>'style1',
							'Grid 2'=>'style2',
		...
		private function elementMap(){
			...
			vc_add_param( 'vc_gmaps', array(
					"type" => "dropdown",
					"heading" => __("Map Type", TEXTDOMAIN),
					"param_name" => "type",
					"value" => array(
								'roadmap'=>'ROADMAP',
								'hybrid'=>'HYBRID',
					...
					"admin_label" => true,
					"description" => __("Select Map Type.", TEXTDOMAIN)
				)
		    );
