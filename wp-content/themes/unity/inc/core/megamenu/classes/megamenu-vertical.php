...
if(!class_exists('Wpo_Megamenu_Vertical')){
class Wpo_Megamenu_Vertical extends Walker_Nav_Menu {
   ...
   public  function start_lvl(  &$output, $depth = 0, $args = array(), $megaconfig=null ) {
		...
		if( isset($megaconfig->subwidth) &&  $megaconfig->subwidth ){
			$attrw .= ' style="width:'.esc_attr( $megaconfig->subwidth ).'px"' ;
		}
		...
		if( isset($megaConfig) && isset($megaConfig->rows) ){
			if( isset($megaConfig->rows[0]->cols) ){
				...
				foreach( $megaConfig->rows[0]->cols as $col ){
					...
					$colclass = isset($col->colclass)?$col->colclass:'';
			 		$megaCols .= '<div class="mega-col col-md-'.esc_attr( $col->colwidth ).' '.esc_attr( $colclass ).'" '.$this->getColumnDataConfig( $col ).'> <div class="mega-col-inner">';
				...
				foreach( $megaConfig->rows as $row ){
					$megaRows .= '<div class="row">';
						foreach( $row->cols as $col ){
							$colclass = isset($col->colclass)?$col->colclass:'';
							 $megaRows .= '<div class="mega-col col-md-'.esc_attr( $col->colwidth ).' '.esc_attr( $colclass ).'" '.$this->getColumnDataConfig( $col ).'> <div class="mega-col-inner">';
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		$text_customize = '';
		if( ! empty( $item->text_customize ) ){
			switch ( $item->text_customize ) {
			 	case 'text_new':
			 		$text_customize  = __('New', TEXTDOMAIN);
		 			break;

		 		case 'text_hot':
		 			$text_customize  = __('Hot', TEXTDOMAIN);
	 				break;

	 			case 'text_featured':
			 		$text_customize  = __('Featured', TEXTDOMAIN);
		 			break;
			...
			$text_customize  = ! empty( $item->text_customize ) ? '<span class="sub-text-label ' . esc_attr( $item->text_customize ) . '">'.esc_html( $text_customize ).'</span>' : '';
		}	

		$item_output = $args->before;
		$item_output .= '<a'. $attributes .'>';
		if($depth==0){
			if(isset($item->megaconfig->icon) && $item->megaconfig->icon !=''){
				$item_output.='<i class="fa '.esc_attr( $item->megaconfig->icon ).'"></i>';
			}
		}
		...
		if($depth==0){
			if(isset($item->megaconfig->description) && $item->megaconfig->description !=''){
				$item_output.='<span class="menu-desc">'.$item->megaconfig->description.'</span>';
			}
		}
	...
	protected function renderAttrs( $menu, $args ,$level){
		if( $this->isLiveEdit ) {
			...
			$t .= ' data-icon="'.(isset($menu->megaconfig->icon)?$menu->megaconfig->icon:"").'"';
			$t .= ' data-description="'.(isset($menu->megaconfig->description)?$menu->megaconfig->description:"").'"';
			return $t;
		}else{
			$t = sprintf( ' data-id="%s"   ', $menu->ID );
	...
	protected function genMegaMenuByConfig( $menu , $level  ){
		...
		$output = '';
			if( isset($menu->megaconfig->subwidth) &&  $menu->megaconfig->subwidth ){
				$attrw .= ' style="width:'.esc_attr( $menu->megaconfig->subwidth ).'px"' ;
			}
			$class  = 'dropdown-menu mega-dropdown-menu';
			if(count($menu->megaconfig->rows[0]->cols)>0){
				$output .= '<div class="'.esc_attr( $class ).'" '.$attrw.' ><div class="dropdown-menu-inner">';

				foreach( $menu->megaconfig->rows  as $row ){
					$output .= '<div class="row">';
						foreach( $row->cols as $col ){
							$colclass = isset($col->colclass)?$col->colclass:'';
							 $output .= '<div class="mega-col col-md-'.esc_attr( $col->colwidth ).' '.esc_attr( $colclass ).'" '.$this->getColumnDataConfig( $col ).'> <div class="mega-col-inner">';
			...
			if( !empty($widgets) ){
				...
				foreach( $widgets as $wid ){
					...
					if( $o ){
						...
						$output .= '<div id="wid-vertical-'.$wid.'" class="wpo-widget">'.$content.'</div>';
					}
				}
				return $output;
			}
