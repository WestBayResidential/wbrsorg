...
class Wpo_Megamenu extends Walker_Nav_Menu {
	...
	public function __construct(  ) {
		...
		if( $this->isLiveEdit && isset($_GET['group']) && $_GET['group'] ){
			...
			$menugroup =  isset($locations['mainmenu'])?$locations['mainmenu']:0;
		}
		$a = get_option( "WPO_MEGAMENU_DATA-".$menugroup) ;
		if( $a ) {
   ...
   public  function start_lvl(  &$output, $depth = 0, $args = array(), $megaconfig=null ) {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if( ! empty( $item->text_customize ) ){
			...
			$text_customize  = ! empty( $item->text_customize ) ? '<span class="' . esc_attr( $item->text_customize ) . ' text-label">'.esc_html( $text_customize ).'</span>' : '';
		}
				...
				$output .= '<div class="row">';
					foreach( $row->cols as $col ){
						...
						$colwidth = isset($col->colwidth) ? $col->colwidth:'';
						 $output .= '<div class="mega-col col-md-'.esc_attr( $colwidth ).' '.esc_attr( $colclass ).'" '.$this->getColumnDataConfig( $col ).'> <div class="mega-col-inner">';
