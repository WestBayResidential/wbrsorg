...
class WPO_LiveTheme {
	...
	public function ajax_upload_pattern(){
		$error = "";
		$msg = "";
		$url = get_template_directory(). '/images/bg/';
		...
		echo "{";
		echo 				"linkimage:'" .get_template_directory_uri()."/images/bg/" .$_FILES['filename']['name'] ."',\n";
	...
	public function ajax_load_pattern(){
		...
		?>
			<div class="col-sm-2 pattern">
				<div class="bg" style="background:url(<?php echo get_template_directory_uri(); ?>/images/bg/<?php echo esc_attr( $pattern ); ?>);"></div>
				<a href="#" data-name="<?php echo esc_attr( $pattern ); ?>" class="remove-pattern"></a>
			</div>
		<?php
		}
		echo '</div>';
	...
	public function ajax_delete_pattern(){
		$file = get_template_directory() . '/images/bg/' . $_POST['pattern'];
		if(is_file($file)){
			echo unlink($file);
		}else{
			echo false;
	...
	public function initScripts(){
		wp_enqueue_script('WPO_bootstrap_js', WPO_THEME_URI.'/js/bootstrap.min.js');
		wp_enqueue_script('WPO_livetheme_js', WPO_FRAMEWORK_ADMIN_STYLE_URI.'/js/livetheme.js');
		wp_enqueue_style('WPO_livetheme_css', WPO_FRAMEWORK_ADMIN_STYLE_URI.'/css/livetheme.css');
		wp_enqueue_style('WPO_bootstrap_css', WPO_THEME_URI.'/css/bootstrap.css');
		wp_enqueue_style('WPO_awesome_css', WPO_THEME_URI.'/css/font-awesome.css');
		
		wp_enqueue_style('WPO_colorpicker_css', WPO_FRAMEWORK_ADMIN_STYLE_URI.'/css/colorpicker.css');
		wp_enqueue_script( 'WPO_colorpicker_js',WPO_FRAMEWORK_ADMIN_STYLE_URI.'/js/colorpicker.js');
	}
	...
	public function ajax_renderLayout(){
		...
		require(WPO_FRAMEWORK_PATH.'livetheme/editor.php');
		exit();
	}
