...
class Wpo_Crowdfunding_Backers_Widget extends WPO_Widget {
	...
	public function __construct() {
		parent::__construct(
			...
			__( 'WPO Campaign Backers', TEXTDOMAIN),
			// Widget description
			array( 'description' => __( 'Display a campaign\'s backers.', TEXTDOMAIN ), )
		);
	}
	...
	public function form( $instance ) {
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id('title') ); ?>"><?php _e('Title:', TEXTDOMAIN) ?>
                <input class="widefat" id="<?php echo esc_attr( $this->get_field_id('title') ); ?>" name="<?php echo esc_attr( $this->get_field_name('title') ); ?>" type="text" value="<?php echo esc_attr($title); ?>" />
            </label>
        </p> 

        <p>
            <label for="<?php echo esc_attr( $this->get_field_id('campaign_id') ); ?>"><?php _e('Campaign:', TEXTDOMAIN) ?>        
            	<select name="<?php echo esc_attr( $this->get_field_name('campaign_id') ) ?>">
            		<option value="current"><?php _e( 'Campaign currently viewed', TEXTDOMAIN ) ?></option>
            		<optgroup label="<?php _e( 'Specific campaigns', TEXTDOMAIN ) ?>">
        ...
        <p>
			<label for="<?php echo esc_attr( $this->get_field_id('number') ); ?>"><?php _e( 'Number of backers to show:', TEXTDOMAIN ); ?></label>
			<input id="<?php echo esc_attr( $this->get_field_id('number') ); ?>" name="<?php echo esc_attr( $this->get_field_name('number') ); ?>" type="text" value="<?php echo esc_attr( $number ); ?>" size="3" />
		</p>

        <p>
			<label for="<?php echo esc_attr( $this->get_field_id('show_name') ) ?>"><?php _e( 'Show backer\'s name:', TEXTDOMAIN ) ?></label>
        ...
        <p>
			<label for="<?php echo esc_attr( $this->get_field_id('show_pledge') ) ?>"><?php _e( 'Show backer\'s pledge amount:', TEXTDOMAIN ) ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('show_location') ) ?>"><?php _e( 'Show backer\'s location:', TEXTDOMAIN ) ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('hide_if_no_backers') ) ?>"><?php _e( 'Hide if there are no backers:', TEXTDOMAIN ) ?></label>
