...
add_action('admin_head', array('WPO_MetaBox', '_global_head'));

add_action('admin_footer', array('WPO_MetaBox', '_global_foot'));

define('WPO_META_MODE_ARRAY', 'array');
define('WPO_META_MODE_EXTRACT', 'extract');
define('WPO_META_FIELD_HINT_TEXT', 'text');
define('WPO_META_FIELD_HINT_TEXTAREA', 'textarea');
define('WPO_META_FIELD_HINT_CHECKBOX', 'checkbox');
define('WPO_META_FIELD_HINT_CHECKBOX_MULTI', 'checkbox_multi');
define('WPO_META_FIELD_HINT_RADIO', 'radio');
define('WPO_META_FIELD_HINT_SELECT', 'select');
define('WPO_META_FIELD_HINT_SELECT_MULTI', 'select_multi');
// depreciated, use WPO_META_FIELD_HINT_SELECT_MULTI instead
define('WPO_META_FIELD_HINT_SELECT_MULTIPLE', 'select_multiple');

define('WPO_META_LOCK_TOP', 'top');
define('WPO_META_LOCK_BOTTOM', 'bottom');
define('WPO_META_LOCK_BEFORE_POST_TITLE', 'before_post_title');
define('WPO_META_LOCK_AFTER_POST_TITLE', 'after_post_title');
define('WPO_META_VIEW_START_OPENED', 'opened');
define('WPO_META_VIEW_START_CLOSED', 'closed');
define('WPO_META_VIEW_ALWAYS_OPENED', 'always_opened');
class WPO_MetaBox
{
	/**
	 ...
	 * WPO_META_MODE_ARRAY (default) and WPO_META_MODE_EXTRACT, this option
	 * should be used when instantiating the class.
	 *
	 * @since	1.2
	 * @access	public
	 * @var		string
	 */
	var $mode = WPO_META_MODE_ARRAY;

	/**
	 * When the mode option is set to WPO_META_MODE_EXTRACT, you have to take
	...
	function WPO_MetaBox($arr)
	{
		...
		if (is_array($arr))
		{
			...
			if ($this->lock_on_top) $this->lock = WPO_META_LOCK_TOP;
			elseif ($this->lock_on_bottom) $this->lock = WPO_META_LOCK_BOTTOM;
	...
	{
		if (WPO_META_MODE_ARRAY == $this->mode AND $key == $this->id)
		{
	...
	function _init()
	{
		...
		if ( ! WPO_MetaBox::_is_post() AND ! WPO_MetaBox::_is_page()) return;
	...
	function _foot()
	{
		...
		)
		{
			...
			(function($){ /* not using jQuery ondomready, code runs right away in footer */
				...
				<?php if (WPO_META_LOCK_TOP == $this->lock): ?>
				<?php if ('side' == $this->context): ?>
				var id = 'wpometa-side-top';
				if ( ! $('#'+id).length)
				{
					$('<div></div>').attr('id',id).prependTo('#side-info-column');
				}
				<?php else: ?>
				var id = 'wpometa-content-top';
				if ( ! $('#'+id).length)
				{
				...
				<?php endif; ?>
				$('#'+id).append(mb);
				<?php elseif (WPO_META_LOCK_BOTTOM == $this->lock): ?>
				<?php if ('side' == $this->context): ?>
				var id = 'wpometa-side-bottom';
				if ( ! $('#'+id).length)
				{
				...
				<?php endif; ?>
				$('#'+id).append(mb);
				<?php elseif (WPO_META_LOCK_BEFORE_POST_TITLE == $this->lock): ?>
				<?php if ('side' != $this->context): ?>
				var id = 'wpometa-content-bpt';
				if ( ! $('#'+id).length)
				{
				...
				$('#'+id).append(mb);
				<?php endif; ?>
				<?php elseif (WPO_META_LOCK_AFTER_POST_TITLE == $this->lock): ?>
				<?php if ('side' != $this->context): ?>
				var id = 'wpometa-content-apt';
				if ( ! $('#'+id).length)
				{
				...
				<?php if (WPO_META_VIEW_START_OPENED == $this->view): ?>
				mb.removeClass('closed');
				<?php elseif (WPO_META_VIEW_START_CLOSED == $this->view): ?>
				mb.addClass('closed');
				<?php elseif (WPO_META_VIEW_ALWAYS_OPENED == $this->view): ?>
	...
	{
		$prefix = 'wpometa_filter_' . $this->id . '_';
	...
	{
		$prefix = 'wpometa_action_' . $this->id . '_';
		$prefix = preg_replace('/_+/', '_', $prefix);
	...
	{
		if ('post' == WPO_MetaBox::_is_post_or_page())
		{
			return TRUE;
		}
	...
	{
		if ('page' == WPO_MetaBox::_is_post_or_page())
		{
			return TRUE;
		}
	...
	{
		$post_type = WPO_MetaBox::_get_current_post_type();
	...
	static function _get_current_post_type()
	{
		...
		if ( isset( $uri ) )
		{
			...
			{
				$post_id = WPO_MetaBox::_get_post_id();
	...
	static function _global_head()
	{
		...
		jQuery(function($)
		{
			...
			$('[class*=docopy-]').click(function(e)
			{
				...
				$.wpometa.trigger('wpa_copy', [the_clone]);
			});
	...
	static function _global_foot()
	{
		...
		(function($){ /* not using jQuery ondomready, code runs right away in footer */
			...
			$.wpometa = $('<div></div>').attr('id','wpometa').appendTo('body');
	...
	{
		if (!$this->in_template AND $this->mode == WPO_META_MODE_EXTRACT)
		{
		...
		$hints = array
		(
			WPO_META_FIELD_HINT_CHECKBOX_MULTI,
			WPO_META_FIELD_HINT_SELECT_MULTI,
			WPO_META_FIELD_HINT_SELECT_MULTIPLE,
		);
	...
	{
		if ($this->is_selected($n, $v)) return 'yes';
		else return 'no';
	}
	...
	{
		echo trim( $this->get_the_checkbox_state($n, $v) );
	}
	...
	{
		echo trim( $this->get_the_select_state($n, $v) );
	}
	...
	{
		echo trim( $this->get_the_group_open($t) );
	}
	...
	{
		echo trim( $this->get_the_group_close() );
	}
	...
	function _save($post_id) 
	{
		...
		if ($this->mode == WPO_META_MODE_EXTRACT)
		{
			$new_fields = array();
			...
			// keep data tidy, delete values if previously using WPO_META_MODE_ARRAY
		...
		else
		{
			...
			// keep data tidy, delete values if previously using WPO_META_MODE_EXTRACT
	...
	//show tabs
	function getTabsConfig($elements){
		$count = 1;
	?>
		<ul class="wpo wpo-meta-tabs">
		<?php
			foreach($elements as $element):
		?>
				<li class="tabs-<?php echo esc_attr( $element['id'] ); ?><?php echo ($count==1)? ' active': ''?>">
					<a href="#<?php echo esc_attr( $element['id'] ); ?>">
						<i class="fa <?php echo esc_attr( $element['icon'] ); ?>"></i>
	...
	function getSelectElement($item) {
		$this->the_field( $item['id'] );
		$value_select = $this->the_value();
		if(empty($value_select)) $value_select = $item['default'];
	?>
        <label><?php echo esc_html( $item['title'] ); ?></label>
        <select name="<?php $this->the_name(); ?>" id="<?php echo esc_attr( $item['id'] ); ?>">
	        <?php foreach($item['data'] as $layout){ ?>
	            <option value="<?php echo esc_attr( $layout['id'] ); ?>" <?php echo ($value_select) == $layout['id'] ? " selected='selected'" : " " ?>><?php echo esc_html( $layout['name'] ); ?></option>
	        <?php } ?>
        </select>
	...
	function getCheckboxElement($item){
		$this->the_field( $item['id'] );
		$value_checkbox = $this->the_checkbox_state($item['id'], 'yes');
		if(empty($value_checkbox)) $value_checkbox = $item['default'];
	?>
        <label><?php echo esc_html( $item['title'] ); ?></label>
        <label for="meta-checkbox">
            <input type="checkbox" name="<?php $this->the_name(); ?>" id="<?php echo esc_attr( $item['id'] ); ?>" value="yes" <?php echo ($value_checkbox=='yes')?'checked': '';  ?>>
            <?php _e( 'Enable', TEXTDOMAIN )?>
        </label>
    <?php
	}

	// add radio type element
	function addRadioElement($item) {
		$this->the_field( $item['id'] );
	?>
		<label><?php echo esc_html( $item['title'] ); ?></label>
            <?php foreach($item['radios'] as $key=>$value) :?>
                <span for="meta-radio-<?php echo esc_attr( $key ); ?>">
                    <input type="radio" name="<?php $this->the_name(); ?>" id="title_style-<?php echo esc_attr( $key ); ?>" value="<?php echo esc_attr( $key ); ?>" <?php $this->the_radio_state( $item['id'], $key ); ?>>
	...
	// add text element
	function addTextElement($item) {
		$this->the_field($item['id']);
		$value = $this->the_value();
		if(empty( $value)) $value = $item['default'];
	?>
		<label><?php echo esc_html( $item['title'] ); ?></label>
        <input type="text" name="<?php $this->the_name(); ?>" style="width: 500px" id="<?php echo esc_attr( $item['id'] );?>" value="<?php echo esc_attr( $value ); ?>"/>
        <br/><?php echo ( $item['des'] ); ?>
    <?php
	}

	// add text element
	function addNumberElement($item) {
		$this->the_field($item['id']);
		$value = $this->the_value();
		if(empty( $value)) $value = $item['default'];
	?>
		<label><?php echo esc_html( $item['title'] ); ?></label>
        <input type="number" name="<?php $this->the_name(); ?>" style="width: 100px" id="<?php echo esc_attr( $item['id'] );?>" value="<?php echo esc_attr( $value ); ?>"/>
        <br/><?php echo ( $item['des'] ); ?>
        <p>
    <?php
	}
	...
	function addTextareaElement($item) {
		$this->the_field($item['id']);
		$_textarea = $this->the_value();
		if(empty( $_textarea)) $_textarea = $item['default'];
	?>
		<label><?php echo esc_html( $item['title'] ); ?></label>
        <textarea class="textarea" style="width: 500px; height: 100px" name="<?php $this->the_name(); ?>" id="<?php echo esc_attr( $item['id'] ) ?>"><?php echo esc_textarea( $_textarea ); ?></textarea>
	<?php
	}
	...
	?>
		<label><?php _e( 'Gallery image: ', TEXTDOMAIN)?></label>
        <label id="wp-content-media-buttons">
            <a href="#" class="button insert-media add_media" data-editor="content" title="Add Media">
                <span class="wp-media-buttons-icon"></span><?php _e('Add Media', TEXTDOMAIN); ?>
            </a>
        </label>
    <?php
	}
...
?>
	    ...
	    <?php 
	    	$this->the_field( $item['id']);
	    	$value_layout = $this->the_value();
			if(empty($value_layout)) $value_layout = $item['default'];
			$url_images = array(
				'0-1-0' => '1col',
				'1-1-0' => '2cl',
				'0-1-1' => '2cr',
    	...
    	?>
		    ...
		    <br>
		    <br>
		    <?php foreach($url_images as $vl=>$img ){ ?>
				<img src="<?php echo WPO_FRAMEWORK_ADMIN_IMAGE_URI . $img.'.png'; ?>" class="layout" data-value="<?php echo esc_attr( $vl ); ?>" alt="">
			<?php } ?>
		    <select name="<?php $this->the_name(); ?>" style="display:none;">
		    <?php foreach ($layouts as $key => $value) { ?>
		    	<option value="<?php echo esc_attr($key); ?>" <?php echo ($value_layout) == $key ? " selected='selected'" : " " ?> ><?php echo esc_html( $value ); ?></option>
	    	<?php } ?>
	    	</select>
		</p>
<?php
