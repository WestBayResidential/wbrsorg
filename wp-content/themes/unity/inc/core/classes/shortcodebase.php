...
abstract class WPO_Shortcode_Base{
	...
	<?php
		foreach($this->options as $option){
			...
			if( isset($option['explain']) && $option['explain'] ){
	...
	?>
			...
			<button type="button" class="btn btn-primary wpo-button-save"><?php echo __('Save', TEXTDOMAIN); ?></button>
			<button type="button" class="btn btn-default wpo-button-back"><?php echo __('Back to list', TEXTDOMAIN); ?></button>
	...
	private function renderUploader( $_id, $_name ) {
		...
		$output .= '<input id="' . esc_attr( $id ) . '" class="upload" type="text" name="'.esc_attr( $name ).'" placeholder="' . __('No file chosen', TEXTDOMAIN) .'" />' . "\n";
		if ( function_exists( 'wp_enqueue_media' ) ) {
			if ( ( $value == '' ) ) {
				$output .= '<input id="upload-' . esc_attr( $id ) . '" class="upload-button button" type="button" value="' . __( 'Upload', TEXTDOMAIN ) . '" />' . "\n";
			} else {
				$output .= '<input id="remove-' . esc_attr( $id ) . '" class="remove-file button" type="button" value="' . __( 'Remove', TEXTDOMAIN ) . '" />' . "\n";
			}
		} else {
