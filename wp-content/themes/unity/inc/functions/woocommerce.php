...
function wpo_woocomerce_icon_wishlist( $value='' ){
	return '<i class="fa fa-heart-o"></i><span>'.__('Wishlist',TEXTDOMAIN).'</span>';
}
    ...
    if(WPO_WOOCOMMERCE_ACTIVED){
        ...
        ?>
            <div id="cart" class="dropdown">
                ...
                <a class="dropdown-toggle mini-cart" data-toggle="dropdown" aria-expanded="true" role="button" aria-haspopup="true" data-delay="0" href="#" title="<?php _e('View your shopping cart', 'woothemes'); ?>">
                    <span class="title-cart"><?php _e('My Cart ',TEXTDOMAIN); ?></span>
...
function wpo_woocommerce_pagination( $per_page,$total ){
    ?>
    <div class="clearfix">
        <?php wpo_pagination($prev = __('Previous',TEXTDOMAIN), $next = __('Next',TEXTDOMAIN), $pages='',array('class'=>'pull-left pagination-sm')); ?>
        <?php global  $wp_query; ?>
...
if( wpo_theme_options('wc_cartnotice') ) {
    ...
    function WPO_jsWoocommerce(){
        ...
        wp_localize_script('wc-add-to-cart','woocommerce_localize',array(
            'cart_success'=> __( wpo_theme_options('wc_cartnotice_text', 'Success: Your item has been added to cart!') ),
        ));
