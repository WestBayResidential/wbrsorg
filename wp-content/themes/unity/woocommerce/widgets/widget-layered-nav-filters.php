...
class WPO_Widget_Layered_Nav_Filters extends WC_Widget_Layered_Nav_Filters {
	...
	public function widget( $args, $instance ) {
		...
		if ( count( $_chosen_attributes ) > 0 || $min_price > 0 || $max_price > 0 ) {
			...
			if ( $min_price ) {
				...
				echo '<li class="chosen"><a title="' . __( 'Remove filter', 'woocommerce' ) . '" href="' . ( $link ) . '">' . __( 'Min', 'woocommerce' ) . ' ' . wc_price( $min_price ) . '</a></li>';
			}

			if ( $max_price ) {
				$link = esc_url(remove_query_arg( 'max_price' ) );
				echo '<li class="chosen"><a title="' . __( 'Remove filter', 'woocommerce' ) . '" href="' . ( $link ) . '">' . __( 'Max', 'woocommerce' ) . ' ' . wc_price( $max_price ) . '</a></li>';
			}
...
register_widget( 'WPO_Widget_Layered_Nav_Filters' );