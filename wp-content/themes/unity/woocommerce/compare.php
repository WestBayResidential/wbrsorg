...
<table class="table table-bordered compare-list" cellpadding="0" cellspacing="0"<?php if ( empty( $products ) ) echo ' style="width:100%"' ?>>
    <thead>
    <tr>
        <th>&nbsp;</th>
    ...
    <?php else : ?>
                        ...
                        switch( $field ) {
                            ...
                            case 'add-to-cart':
                                echo apply_filters( 'woocommerce_loop_add_to_cart_link',
                                    sprintf( '<a href="%s" rel="nofollow" data-product_id="%s" data-product_sku="%s" class="button btn-outline %s product_type_%s">%s</a>',
        ...
        <?php if ( $repeat_add_to_cart == 'yes' && isset( $fields['add-to-cart'] ) ) : ?>
            <tr class="add-to-cart repeated">
                ...
                <?php foreach( $products as $i => $product ) : $product_class = ( $i % 2 == 0 ? 'odd' : 'even' ) . ' product_' . $product->id ?>
                    <td class="<?php echo esc_attr( $product_class ) ?>"><?php $wc_get_template( 'loop/add-to-cart.php', array( 'product' => $product ) ); ?></td>
                <?php endforeach; ?>
    ...
    jQuery(document).ready(function($){
        ...
        $('body').on( 'yith_woocompare_render_table', function(){
            if( $( window ).width() > 767 ) {
                ...
                $('<table class="table table-bordered compare-list" />').insertAfter( $('h1') ).hide();
            }
