			...
			<div class="campaign-summary cf">		
				...
				<?php else : ?>
					...
					<?php if ( $campaign->is_funded() ) : ?>
						<h3 class="campaign-ended"><?php printf( __( 'This campaign successfully reached its funding goal and ended %s',TEXTDOMAIN ), '<span class="time-ago">'.esc_html( sofa_crowdfunding_get_time_since_ended($campaign, true) ).'</span>' ) ?></h3>
					<?php else : ?>
						<h3 class="campaign-ended"><?php printf( __( 'This campaign failed to reach its funding goal %s',TEXTDOMAIN ), '<span class="time-ago">'.esc_html( sofa_crowdfunding_get_time_since_ended($campaign, true).'</span>' ) ) ?></h3>
					<?php endif ?>
