...
function sc_banner($atts, $content=null){	
	...
	return empty($src) ? '' : ('<a href="' . (empty($link) ? '#' : esc_url($link)) . '"' . ($id ? ' id="' . $id . '"' : '')
...
function sc_button($atts, $content=null){	
	...
	$s = getStyleString($top, $right, $bottom, $left);
	$class .= getStyleMargin($top, $right, $bottom, $left);
	$class .= ($curve=="off" || $curve=="no" ? ' curve_no' : '');
	...
	return '<'.(sc_param_is_on($block) ? 'div' : 'span').' class="sc_button sc_button_style_' . $style . ' sc_button_size_' . $size . ($align && $align!='none' ? ' align'.$align : '') . (!empty($class) ? ' '.$class : '')
...
function sc_columns($atts, $content=null){	
	...
	return '<div' . ($id ? ' id="' . $id . '"' : '') . ' class="'. (sc_param_is_off($indentation) ? 'columnsFloat ' : '') .''. (sc_param_is_on($autoheight) ? 'autoHeight ' : '') .'columnsWrap sc_columns sc_columns_count_' . $count . (!empty($class) ? ' '.$class : '') . '"'.($s!='' ? ' style="'.$s.'"' : '').'>' . do_shortcode($content).'</div>';
}
...
function sc_contact_form($atts, $content = null) {
	...
	if($style!='order') {
		...
		return '<div ' . ($id ? ' id="' . $id . '"' : '') . 'class="'.($style_color=='dark' ? 'dark' : 'light').' sc_contact_form sc_contact_form_' . ($content ? 'custom' : 'contact') . (!empty($class) ? ' ' . $class : '') . '"' . ($s != '' ? ' style="' . $s . '"' : '') . '>'
		...
		. ($content != ''
			...
			. '<label class="required" for="sc_contact_form_username">' . __('Name', 'themerex') . '</label><input placeholder="' . __('Your name', 'themerex') . '" id="sc_contact_form_username" type="text" name="username">'
			. '</div>'
			. '<div class="columns1_2">'
			. '<label class="required" for="sc_contact_form_email">' . __('E-mail', 'themerex') . '</label><input placeholder="' . __('Your e-mail', 'themerex') . '" id="sc_contact_form_email" type="text" name="email">'
			. '</div>'
			. '</div>'
			. '<div class="message">'
			. '<label class="required" for="sc_contact_form_message">' . __('Message', 'themerex') . '</label><textarea id="sc_contact_form_message" class="textAreaSize" name="message"></textarea>'
			. '</div>'
			. '<div class="sc_contact_form_button">'
			. '<div class="squareButton sc_button_style_accent_2 sc_button_size_big accent_2 big"><a href="#" class="sc_contact_form_submit">' . __('Send the message', 'themerex') . '</a></div>'
			. '</div>'
		)
		...
		return  '<div class="'.($style_color=='dark' ? 'dark' : 'light').' sc_contact_form sc_contact_form_order' . (!empty($class) ? ' ' . $class : '') . '"' . ($s != '' ? ' style="' . $s . '"' : '') . '>'
		. ($title ? '<h1 class="title">' . $title . '</h1>' : '')
		. ($description ? '<div class="description">' . $description . '</div>' : '')
		. '<form class="simform" autocomplete="off"' . ($id ? ' id="' . $id . '"' : '') . ' data-formtype="okorder" method="post" action="' . ($action ? $action : $THEMEREX_ajax_url) . '">'
			.'<div class="simform-inner">'
				.'<ol class="questions">'
							...
							.'</li>'
							.'<li class="dot">'
								.'<span><label for="q2">Your company?</label></span>'
								.'<input placeholder="' . __('Company', 'themerex') . '" id="q2" name="q2" type="text"/>'
							.'</li>'
							.'<li class="dot">'
								.'<span><label for="q3">Your email address?</label></span>'
								.'<input placeholder="' . __('Email', 'themerex') . '" id="q3" name="q3" type="text"/>'
							.'</li>'
							.'<li class="dot">'
								.'<span><label for="q4">What kind of project?</label></span>'
								.'<input placeholder="' . __('Project type', 'themerex') . '" id="q4" name="q4" type="text"/>'
							.'</li>'
							.'<li class="dot">'
								.'<span><label for="q5">What\'s your budget?</label></span>'
								.'<input placeholder="' . __('Ballpark budget', 'themerex') . '" id="q5" name="q5" type="text"/>'
							.'</li>'
							.'<li class="dot">'
								.'<span><label for="q6">Tell us about your project</label></span>'
								.'<input placeholder="' . __('Objectives', 'themerex') . '" id="q6" name="q6" type="text"/>'
							.'</li>'
				.'</ol><!-- /questions -->'
		...
		.'<div class="continue_button sc_button sc_button_style_border sc_button_size_big squareButton border big">'
			.'<a href="#">Continue</a>'
		.'</div>'
		. '</form>
...
function sc_emailer($atts, $content = null) {
	...
	return '<div' . ($id ? ' id="' . $id . '"' : '').' class="sc_emailer inputSubmitAnimation' . ($align && $align!='none' ? ' sc_align' . $align : '') . (sc_param_is_on($open) ? ' sFocus opened' : ' radCircle') . (!empty($class) ? ' '.$class : '') . '"' . ($s ? ' style="'.$s.'"' : '') . '>'
...
function sc_icon($atts, $content=null){	
	...
	$s2 = ($weight != '' ? 'font-weight:'. $weight.';' : '')
		...
		. ($color != '' ? 'color:'.$color.';' : '')
		. ($bg_color != '' ? 'background-color:'.$bg_color.';' : '')
		. ($background == 'round' && (int) $size > 0 ? ($s ? '' : 'display:inline-block;') . 'width:' . round($size*3) . 'px;height:' . round($size*3) . 'px;line-height:' . round($size*3) . 'px;' : '')
	;
	return $icon!='' 
...
function sc_image($atts, $content=null){	
	...
	if(sc_param_is_on($link)){
		return empty($src) || empty($url_color) ? '' : ('<a href="'.esc_url(trim($href)).'"' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_image sc_image_hover' . ($align && $align != 'none' ? ' sc_image_align_' . $align : '') . (!empty($shape) ? ' sc_image_shape_' . $shape : '') . (!empty($class) ? ' ' . $class : '') . '"' . ($s != '' ? ' style="' . $s . '"' : '') . '>'
			. '<img src="' . $src . '" alt="" />'
		...
		return empty($src) ? '' : ('<figure data-image="' . $hoverSrc . '" data-title="' . trim($title) . '"' . ($id ? ' id="' . $id . '"' : '') . ' class="thumb hoverIncrease sc_image ' . ($align && $align != 'none' ? ' sc_image_align_' . $align : '') . (!empty($shape) ? ' sc_image_shape_' . $shape : '') . (!empty($class) ? ' ' . $class : '') . '"' . ($s != '' ? ' style="' . $s . '"' : '') . '>'
...
function sc_price($atts, $content=null){	
	...
	if (!empty($money)) {
		...
		if (count($m)==1) $m[1] = '';
		$output = '
			<div' .($s!='' ? ' style="'.$s.'"' : '').''. ($id ? ' id="' . $id . '"' : '').' class="sc_price_item'. (!empty($class) ? ' '.$class : '').'">
...
function sc_price_table($atts, $content=null){	
	...
	$THEMEREX_sc_price_table_counter = 0;
	$count = max(1, $count);
	return '<div' . ($id ? ' id="' . $id . '"' : '') . ' class="'.($style ? 'sc_pricing_'.$style.' ' : '').'sc_pricing_table columns_' . $count . ($align && $align!='none' ? ' align'.themerex_strtoproper($align) : '') . (!empty($class) ? ' '.$class : '') . '"' . ($s!='' ? ' style="'.$s.'"' : '') . '>'
			. do_shortcode($content)
		. '</div>';
}
...
function sc_price_item($atts, $content=null) {
	...
	return '<div class="'.(sc_param_is_on($active) ? 'active ' : '').'sc_pricing_columns sc_pricing_column_'.$THEMEREX_sc_price_table_counter. (!empty($class) ? ' '.$class : '').'"><ul'.(sc_param_is_on($animation) ? ' class="columnsAnimate"' : '') . ($id ? ' id="' . $id . '"' : '') . '>'
		. do_shortcode($content) 
		. '</ul></div>';
}
...
function sc_quote($atts, $content=null){	
	...
	$content = do_shortcode($content);
	if (themerex_substr($content, 0, 2)!='<p') $content = '<p>' . $content . '</p>';
	return '<blockquote' . ($id ? ' id="' . $id . '"' : '') . $cite_param . ' class="sc_quote'. (!empty($class) ? ' '.$class : '').' sc_quote_style_' . $style.'"' . ($s!='' ? ' style="'.$s.'"' : '') . '>'
		. $content
...
function sc_skills_item($atts, $content=null) {
	...
	if (empty($color)) $color = apply_filters('theme_skin_get_theme_color_2', $color);
	...
	if ($THEMEREX_sc_skills_type=='arc') {
		...
		if (in_array($THEMEREX_sc_skills_type, array('bar')) && $THEMEREX_sc_skills_dir=='horizontal') {
			...
			.'</div>'
			.$title_block
			.'<div class="sc_skills_wrap"><div class="sc_skills_count'. (!empty($class) ? ' '.$class : '').'"' . ($THEMEREX_sc_skills_type=='bar' && $color & $style_bar == 1 ? ' style="background-color:' . $color . ';"' : '') . '>'
			. '</div></div>';
		}
...
function sc_tabs($atts, $content = null) {
	...
	$tabs_output = '<div' . ($id ? ' id="' . $id . '"' : '') . ' class="'.(sc_param_is_on($padding) ? 'padding ' : '').'   sc_tabs sc_tabs_style_'.$style. (!empty($class) ? ' '.$class : '') . '"'.($s!='' ? ' style="'.$s.'"' : '') .' data-active="' . ($initial-1) . '">'
...
function sc_testimonials($atts, $content=null){	
	...
	return
		'<div' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_testimonials sc_testimonials_style'.(sc_param_is_off($controls) ? '' : ' sc_testimonials_controls_'.$controls) . (!empty($class) ? ' '.$class : '') .'"' . ($s!='' ? ' style="'.$s.'"' : '') . '>'
...
function sc_text_slider($atts, $content=null){
	...
	return
	'<div' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_text_slider' . (!empty($class) ? ' '.$class : '') .'"' . ($s!='' ? ' style="'.$s.'"' : '') . '>'
...
function sc_sidebar($atts, $content=null){
	...
	return '<div class="widget_area sidebarStyleLight sc_sidebar_selector'.(!empty($class) ? $class : '').'"'.(!empty($s) ? ' style="'.$s.'"' : '').'>'.$sidebar_content.'</div>';
}
// ---------------------------------- [/trx_sidebar] ---------------------------------------
?>